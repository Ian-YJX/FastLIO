...
#pragma omp parallel for num_threads(kNumOmpCores)
        ...
        const Session &_target_sess, const Session &_source_sess, std::pair<int, int> _rimg_shape)
    {
        ...
        for (std::size_t idx_scan = 0; idx_scan < scans.size(); ++idx_scan)
        {
            ...
            std::vector<int> dynamic_map_point_indexes_by_this_scan = calcDescrepancyAndParseDynamicPointIdx(scan_rimg, diff_rimg, map_rimg_ptidx, diff_thres_for_check_valid_nd);
        ...
        const Session &_target_sess, const Session &_source_sess, float _res_alpha)
    {
        ...
        std::vector<int> dynamic_point_indexes = calcDescrepancyAndParseDynamicPointIdxForEachScanForPD(_target_sess, _source_sess, curr_rimg_shape_);
        ...
        const Session &_target_sess, const Session &_source_sess, float _res_alpha)
    {
        ...
        // map-side removal: remove and get dynamic (will be removed) points' index set
        std::vector<int> dynamic_point_indexes = calcDescrepancyAndParseDynamicPointIdxForEachScanForND(_target_sess, _source_sess, curr_rimg_shape_);
        ROS_INFO_STREAM("\033[1;32m -- The number of dynamic points: " << dynamic_point_indexes.size() << "\033[0m");
        pcl::PointCloud<PointType>::Ptr map_global_curr_dynamic_this_turn(new pcl::PointCloud<PointType>());
        parsePointcloudSubsetUsingPtIdx(map_global_curr, dynamic_point_indexes, map_global_curr_dynamic_this_turn);
        ...
        const Session &_target_sess, const Session &_source_sess, float _res_alpha)
    {
        ...
        auto [map_global_curr_static_this_turn, map_global_curr_dynamic_this_turn] = partitionCurrentMapForND(_target_sess, _source_sess, _res_alpha);
        ...
        const Session &_target_sess, const Session &_source_sess, float _res_alpha)
    {
        ...
        auto [map_global_curr_static_this_turn, map_global_curr_dynamic_this_turn] = partitionCurrentMapForPD(_target_sess, _source_sess, _res_alpha);
    ...
    void Removerter::removeOnce(const Session &_target_sess, const Session &_source_sess, float _res_alpha)
    {
        ...
        auto [map_global_curr_static_this_turn, map_global_curr_dynamic_this_turn] = partitionCurrentMap(_target_sess, _source_sess, _res_alpha);
    ...
    void Removerter::detectLowDynamicPoints(void)
    {
        ...
        ROS_INFO_STREAM("\033[1;32m filterStrongPD() succeeded. strong PD map size: " << query_sess_.map_global_pd_strong_->points.size() << "\033[0m");
        ...
        */
        ...
        {
            ROS_INFO_STREAM("\033[1;32m Saving merged maps for visual debug \033[0m");
            ...
            ROS_INFO_STREAM("\033[1;32m The number of points in union_map_queryside.pcd: " << map_global_union_queryside->points.size() << "\033[0m");
            pcl::io::savePCDFileASCII(save_pcd_directory_ + "union_map_queryside.pcd", *map_global_union_queryside);
            ...
            ROS_INFO_STREAM("\033[1;32m The number of points in union_map_centralside.pcd: " << map_global_union_queryside->points.size() << "\033[0m");
            pcl::io::savePCDFileASCII(save_pcd_directory_ + "union_map_centralside.pcd", *map_global_union_centralside);
