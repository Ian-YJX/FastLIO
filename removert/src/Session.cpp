...
namespace ltremovert
{
    ...
    void Session::loadSessionInfo(std::string _sess_type, std::string _scan_dir, std::string _pose_path)
    {
        ...
        ROS_INFO("\033[1;32m Load session info: %s\033[0m", sess_type_.c_str());
        ROS_INFO("\033[1;32m Scan directory: %s\033[0m", scan_dir_.c_str());
        ROS_INFO("\033[1;32m Pose path: %s\033[0m", pose_path_.c_str());
        // load scan paths
        ...
        ROS_INFO_STREAM("\033[1;32m scan_poses_.size(): " << scan_poses_.size() << "scan_paths_.size()" << scan_paths_.size() << "\033[0m");
    ...
    double xyzDist(const Eigen::Matrix4d &_pose1, const Eigen::Matrix4d &_pose2)
    {
        ...
        return std::sqrt((_pose1(0, 3) - _pose2(0, 3)) * (_pose1(0, 3) - _pose2(0, 3)) + (_pose1(1, 3) - _pose2(1, 3)) * (_pose1(1, 3) - _pose2(1, 3)) + (_pose1(2, 3) - _pose2(2, 3)) * (_pose1(2, 3) - _pose2(2, 3)));
    } // xyzDist
    ...
    void Session::precleaningKeyframes(float _radius)
    {
        for (std::size_t idx_kf = 0; idx_kf < keyframe_scans_.size(); idx_kf++)
        {
            ...
            for (int pt_idx = 0; pt_idx < num_points; ++pt_idx)
            {
                ...
                if (this_point_range < _radius & this_point_z < 0.5 & -0.5 < this_point_z)
                {
                    continue;
                }
